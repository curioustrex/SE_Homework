...
public class AuthorListParser {
	...
	private static final Set<String> AVOID_TERMS_IN_LOWER_CASE = Set.of("jr", "sr", "jnr", "snr", "von", "zu", "van",
			"der");
	...
	private static final int OFFSET_TIKEN_NAME = 2;

	// Character -- token terminator (either " " or
	// "-") comma)
	// Constant HashSet containing names of TeX special characters
	private static final Set<String> TEX_NAMES = Set.of("aa", "ae", "l", "o", "oe", "i", "AA", "AE", "L", "O", "OE",
			"j");
	/**
	 ...
	 * only valid if getToken returns Token.WORD
	 */
	...
	public AuthorList parse(String listOfNames) {
		...
		if (!authorsContainAND && !authorsContainOpeningBrace && !authorsContainSemicolon
			...
			boolean spaceInAllParts = arrayNameList.stream().filter(name -> name.contains(" ")).count() == arrayNameList
					.size();
			...
			if (spaceInAllParts) {
				...
				countValueParts(listOfNames,valuePartsCount,avoidIndex,arrayNameList);
			}
		}
	...
	private String countValueParts(String listOfNames, int valuePartsCount, Collection<Integer> avoidIndex, List<String> arrayNameList) {
	...
	private Optional<Author> getauthor() {
		...
		if (commaFirst < 0) { // no commas
			if (vonStart < 0) { // no 'von part'
				...
				int index = (tokens.size() - (2 * TOKEN_GROUP_LENGTH)) + OFFSET_TIKEN_NAME;
				if (index > 0) {
	 ...
	 * spaces or dashes, depending on stored in 'tokens'. Callers always ensure that
	...
	private String concatTokens(List<Object> tokens, int start, int end, int offset, boolean dotAfter) {
		...
		while (updatedStart < end) {
			result.append(tokens.get((updatedStart - TOKEN_GROUP_LENGTH) + OFFSET_TIKEN_NAME));
	 ...
	 * before the first invocation. Procedure updates <CODE>token_end</CODE>; thus,
	 * subsequent invocations do not require any additional variable settings.
	 * <p>
	 ...
	 * <CODE>token_end</CODE>, <CODE>token_abbr</CODE>, <CODE>token_term</CODE>, and
	 ...
	 *         "And", or "aND", etc.) or a semicolon, <CODE>Token.WORD</CODE> --
	 *         token is a word; additional information is given in global variables
